...
//-----------------------------------------------------------------------------
Max7456::Max7456(SPIClass* spi, byte pinCS) {
  spidevice = spi;
  _pinCS = pinCS;
  // this->init(pinCS);
}
...
void Max7456::setBlinkParams(byte blinkBase, byte blinkDC) {
  ...
  spidevice->transfer(VM1_ADDRESS_WRITE);
  spidevice->transfer(_regVm1.whole);
...
void Max7456::setDisplayOffsets(byte horizontal, byte vertical) {
  ...
  digitalWrite(_pinCS, LOW);
  spidevice->transfer(HOS_ADDRESS_WRITE);
  spidevice->transfer(_regHos.whole);
  spidevice->transfer(VOS_ADDRESS_WRITE);
  spidevice->transfer(_regVos.whole);
  digitalWrite(_pinCS, HIGH);
}
...
void Max7456::sendCharacter(const charact chara, byte x, byte y) {
  ...
  // Datasheet page 38
  digitalWrite(_pinCS, LOW);
  spidevice->transfer(CMAH_ADDRESS_WRITE);
  spidevice->transfer(charAddress);

  for (byte i = 0; i < 54; i++) {
    spidevice->transfer(CMAL_ADDRESS_WRITE);
    spidevice->transfer(i);
    spidevice->transfer(CMDI_ADDRESS_WRITE);
    spidevice->transfer(chara[i]);
  }

  _regCmm = 0xA0; //To write the NVM array
  spidevice->transfer(CMM_ADDRESS_WRITE);
  spidevice->transfer(_regCmm);
  //while STAT[5] is not good, we wait.
  _regStat.bits.characterMemoryStatus = 1;
  while (_regStat.bits.characterMemoryStatus == 1) {
    spidevice->transfer(STAT_ADDRESS_READ);
    _regStat.whole = spidevice->transfer(0x00);
  }
...
void Max7456::getCharacter(charact chara, byte x, byte y) {
  ...
  for (byte i = 0; i < 54; i++) {
    ...
    spidevice->transfer(CMDO_ADDRESS_READ); //read from device through spi
    chara[i] = spidevice->transfer(0x00);
  }
...
void Max7456::printMax7456Chars(byte chars[], byte size, byte x, byte y,
  ...
  digitalWrite(_pinCS, LOW);
  spidevice->transfer(DMM_ADDRESS_WRITE);
  spidevice->transfer(_regDmm.whole);
  // Datasheet page 31
  spidevice->transfer(DMAH_ADDRESS_WRITE); // set start address high
  spidevice->transfer(posAddressHI);

  spidevice->transfer(DMAL_ADDRESS_WRITE); // set start address low
  spidevice->transfer(posAddressLO);

  for (int i = 0; i < size; i++) {
    currentCharMax7456 = chars[i];
    spidevice->transfer(DMDI_ADDRESS_WRITE);
    spidevice->transfer(currentCharMax7456);
  }

  //end character (we're done).
  spidevice->transfer(DMDI_ADDRESS_WRITE);
  spidevice->transfer(0xff);
  digitalWrite(_pinCS, HIGH);
}
...
void Max7456::clearScreen() {
  ...
  while (_regDmm.bits.clearDisplayMemory == 1) {
    spidevice->transfer(DMM_ADDRESS_READ);
    _regDmm.whole = spidevice->transfer(0x00);
  }
...
void Max7456::init() {
  ...
  digitalWrite(_pinCS, LOW);
  spidevice->transfer(VM0_ADDRESS_WRITE);

  _regVm0.whole = 0x00;
  _regVm0.bits.videoSelect = 0; //PAL
  ...
  for (int x = 0; x < 16; x++) {
    ...
    spidevice->transfer(x + RB0_ADDRESS_WRITE);
    spidevice->transfer(_regRb[x].whole);
  }
